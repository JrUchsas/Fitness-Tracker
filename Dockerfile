...
# Install system dependencies, libzip & SQLite/MySQL/PostgreSQL extensions
...
CMD ["sh", "-c", "mkdir -p /var/data && (test -f /var/data/database.sqlite || touch /var/data/database.sqlite) && php artisan migrate --force && php -S 0.0.0.0:${PORT:-8000} -t public/"]
